...
function CipherChainPageContent() {
  ...
  const handleSubmit = async () => {
    ...
    await saveSubmission();
    setSubmitted(true);
    alert("Submission successful! Great work on the cipher chain.");
  };

  return (
    <ChallengeLayout
      challengeTitle="Cipher Chain"
      difficulty="MEDIUM"
      points={180}
    >
        ...
        {showTutorial && (
          <div className="mb-6 rounded-2xl border-2 border-blue-200 bg-blue-50 p-6">
            ...
            <div className="grid gap-6 md:grid-cols-2">
              <div>
                <h4 className="mb-3 font-semibold text-blue-700">Cipher Chaining:</h4>
                <ul className="space-y-2 text-sm text-slate-700">
                  <li>Multiple ciphers applied sequentially</li>
                  <li>Decode in reverse order (last first)</li>
                  <li>Common chains: Base64 → ROT13 → Hex</li>
                  <li>Identify each layer by format</li>
                  <li>Work backwards from final output</li>
                </ul>
              </div>

              <div>
                <h4 className="mb-3 font-semibold text-green-700">Decoding Strategy:</h4>
                <ul className="space-y-2 text-sm text-slate-700">
                  <li className="flex items-start gap-2"><span className="text-green-500">✓</span><span>Identify the outermost encoding</span></li>
                  <li className="flex items-start gap-2"><span className="text-green-500">✓</span><span>Decode and check if readable</span></li>
                  <li className="flex items-start gap-2"><span className="text-green-500">✓</span><span>Continue decoding layers</span></li>
                  <li className="flex items-start gap-2"><span className="text-green-500">✓</span><span>Use CyberChef for chaining</span></li>
                </ul>
              </div>
            </div>
          </div>
        ...
        <div className="rounded-2xl bg-white p-6 shadow">
          <div className="mb-6">
            ...
            <p className="text-slate-700">
              Decode a message that has been encrypted with multiple ciphers in sequence. Identify each layer and decode them in the correct order. 
            ...
            <div className="space-y-4">
              <div>
                ...
                <code className="block rounded-lg bg-white p-3 text-sm font-mono break-all">
                  ZmxhZ3tjaGFpbmVkX2NpcGhlcnN9
                </code>
              </div>
              <div className="mt-4 rounded-lg bg-yellow-50 border border-yellow-200 p-3">
                <p className="text-xs text-yellow-800">
                  <strong>Hint:</strong> This message is Base64 encoded. Decode it to reveal the flag directly.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-6">
            <div>
              ...
              <textarea
                ...
                rows={6}
                placeholder="Describe your multi-layer decoding approach..."
              />
...
export default function CipherChainPage() {
  return (
    <Suspense fallback={
